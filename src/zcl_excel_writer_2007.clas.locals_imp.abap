...
CLASS lcl_create_xl_sheet DEFINITION CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    TYPES: BEGIN OF colors,
             colorrgb type zif_excel_data_decl=>zexcel_color,
           END OF colors,
           BEGIN OF cfvo,
             value type zif_excel_data_decl=>zexcel_conditional_value,
             type  type zif_excel_data_decl=>zexcel_conditional_type,
           END OF cfvo,
...
* Theme not supported yet - start with RGB
  ...
  METHOD add_sheet_views.
    ...
    DATA: lv_value                    TYPE string,
          lv_freeze_cell_row          type zif_excel_data_decl=>zexcel_cell_row,
          lv_freeze_cell_column       type zif_excel_data_decl=>zexcel_cell_column,
          lv_freeze_cell_column_alpha type zif_excel_data_decl=>zexcel_cell_column_alpha.
...
* Always pass through this coding
      IF o_worksheet->zif_excel_sheet_properties~get_style( ) IS NOT INITIAL.
        DATA: lts_sorted_columns TYPE SORTED TABLE OF zif_excel_data_decl=>zexcel_cell_column WITH UNIQUE KEY table_line.
        TYPES: BEGIN OF ty_missing_columns,
                 first_column type zif_excel_data_decl=>zexcel_cell_column,
                 last_column  type zif_excel_data_decl=>zexcel_cell_column,
...
*> End of insertion Issue #572 - Protect sheet with filter caused Excel error
  ...
  METHOD add_autofilter.
    DATA:
      ...
      lv_value       TYPE string,
      lv_column      type zif_excel_data_decl=>zexcel_cell_column,
      lt_values      type zif_excel_data_decl=>zexcel_t_autofilter_values,
      ls_values      type zif_excel_data_decl=>zexcel_s_autofilter_values,
...
* Create node filtercolumn
  ...
  METHOD add_conditional_formatting.
    DATA: lo_element               TYPE REF TO if_ixml_element,
          ...
          lv_value                 TYPE string,
          ls_databar               type zif_excel_data_decl=>zexcel_conditional_databar,      " Databar by Albert Lladanosa
          ls_colorscale            type zif_excel_data_decl=>zexcel_conditional_colorscale,
          ls_iconset               type zif_excel_data_decl=>zexcel_conditional_iconset,
          ls_cellis                type zif_excel_data_decl=>zexcel_conditional_cellis,
          ...
          ls_expression            type zif_excel_data_decl=>zexcel_conditional_expression,
          ls_conditional_top10     type zif_excel_data_decl=>zexcel_conditional_top10,
          ls_conditional_above_avg type zif_excel_data_decl=>zexcel_conditional_above_avg,
  ...
  METHOD add_drawing_for_comments.
    ...
    IF lo_drawing_for_comments->is_empty( ) = abap_false.
      lo_element = o_document->create_simple_element( name   = lc_xml_node_drawing_for_cmt
                                                      parent = o_document ).
      v_relation_id += 1.  " +1 for legacyDrawings
      ...
      v_relation_id += 1.  "for xl/comments.xml (not referenced in XL sheet but counted in sheet rels)
    ENDIF.
* End   - Add - Issue #180
  ENDMETHOD.
