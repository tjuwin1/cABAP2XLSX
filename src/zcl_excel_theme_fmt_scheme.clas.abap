...
CLASS ZCL_EXCEL_THEME_FMT_SCHEME IMPLEMENTATION.
  ...
  METHOD parse_string.
    DATA li_stream   TYPE REF TO if_ixml_istream_core.
    DATA li_ixml     TYPE REF TO if_ixml_core.
    DATA li_document TYPE REF TO if_ixml_document.
    DATA li_factory  TYPE REF TO if_ixml_stream_factory_core.
    DATA li_parser   TYPE REF TO if_ixml_parser_core.
    DATA li_istream  TYPE REF TO if_ixml_istream_core.

    li_ixml = cl_ixml_core=>create( ).
    li_document = li_ixml->create_document( ).
    li_factory = li_ixml->create_stream_factory( ).
    li_istream = li_factory->create_istream_xstring( xco_cp=>string( iv_string )->as_xstring( xco_cp_character=>code_page->utf_8 )->value ).
    ...
    li_parser->parse( ).
    "li_istream->close( ). @TODO: What is this for?
